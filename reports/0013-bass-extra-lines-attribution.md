# report 0013 — the bass extra lines: attribution and the window

The worked example the methods standard uses twice.

**§3 is why the null is chosen before scoring.** Against a *permutation* null —
shuffling offsets within a register — no attribution family beat its own null on
either instrument. Against a naive uniform redraw, the same data manufactured a
+9-point excess out of nothing. The choice of null, not the data, decided whether
there was a finding.

**§4 is why suggestive is not a finding.** The bass flanking-pair symmetry
measures 0.45–0.59 against a null of 0.80–0.83 — the most interesting number in
the document — and is recorded as *suggestive, not a finding*, because 0.45 is a
long way from the 0 a real symmetric sideband pair would give.

## Where this stands

**INVESTIGATION COMPLETE 2026-08-12. No code change.** The
experiments are `cargo lab strobe replay` **E10–E12**, run on both pianos; they
join E1–E9 as standing regression material.

report 0012 §5 kept the bass out of unison assist's claimed scope because keys 0–27
...
unestablished. This report settles what can be settled from the captures we have,
...
constant in cents across partials. report 0012 §8 added that ~78 % of bass rank-2
...
Every synthetic trial behind report 0012 ran the treble's 1024-sample window, whose
...
the whole bank. Taken with report 0012 §8's 78 % full-rate match rate, **the bass
...
having been derived (§3 of report 0012) from an independence that does not hold
here.
...
the variance inflation is **2.0×**. report 0012 §1 conjectured that the effective
...
against a stated criterion (report 0011), and moving it for a second consumer needs
...
limit is reported *at* the limit whatever the truth was (report 0012 §4), and the
...
- **A second string is excluded**, which is what report 0012 §5 already concluded
...
**E11** is the bass-window experiment and its own cheap test: an
...
the shape a soundboard resonance would take, and they are exactly what the soundboard-fingerprint experiment is for. They are too few to explain the register.
...
(report 0012 §5), so "shipping the bass" was never about removing a constant — it is
...
- The markers themselves are honest: real spectral content (§1), reproduced
  across independent strikes to 1 % (report 0012 §7), with the record's own
...
report 0012 §1's decision survives with a better argument on both sides. The fold is
...
muted, then open — which is the mute experiment and the mute test.
...
Two things the mute test should now carry that it did not:
...
- No behaviour changes. `strobe_replay` gains E10–E12; report 0012's E1–E9 are
  untouched and still reproduce.
- report 0012 §5's separate investigation resolves here, and the design note's E-Q
...
- The soundboard-fingerprint experiment gains a starting point it did not have: 170.7 Hz (piano 1) and
...
- **`(f₀, B)` for the neighbour prediction comes from each set's own captures**,
  ...
  rather than regenerated (`capture-sets.md`).
- **E12d's fit has almost no lever arm**, as report 0012 §6 says: a handful of
...
are report 0011's, E6–E9 report 0012's, and none of them move.
