# NSGA-II Methodology & Reproducibility

> The project called this optimiser "MOBO" until 2026-09. The name was wrong on
> the "Bayesian": the search is NSGA-II, an evolutionary non-dominated-sorting
> algorithm, via Optuna. Filenames, the harness subcommand and this document were
> renamed together; anything older that says MOBO means this.
...
validity**. report 0001 gives the conceptual rationale; report 0006 records the findings.
...
read before any conclusion drawn from a NSGA-II run is treated as load-bearing.
...
| --- | --- | --- |
| `tuner-lab/src/engine/nsga2.rs` (`cargo lab engine nsga2`) | Synthetic dataset + objective evaluator (`--serve` stdin/stdout protocol) | modified, **uncommitted** |
| `tuner-lab/scripts/optimize_twm.py` | Optuna NSGA-II orchestrator (5 arms) | **untracked** |
| `tuner-lab/scripts/validate_config.py` | Real-capture validator (the decision gate) | untracked |
| `twm_nsga2.db` | Optuna SQLite study storage (resumable) | untracked |
...
The invocations live with the tool:
[`tuner-lab/README.md`](../../tuner-lab/README.md) § *Running the NSGA-II sweep*,
which also carries the reproduction checklist.

## 3. The synthetic dataset

**Why synthetic (report 0001).** No labelled in-scope acoustic corpus exists with
...
overfit a single instrument otherwise — see report 0006).

### 3.1 Determinism

- Hand-rolled **SplitMix64** RNG (`nsga2_evaluator.rs`), chosen over `rand::StdRng`
...
- **Dataset fingerprint** (FNV-fold over every frame's key, d_cents, and all peak
  ...
  diagnostic mode: `./target/release/tuner-lab engine nsga2` (look for the
...
1. *Production, not separability.* The optimized metric was the all-88 *separability*
   argmin, which in refine mode is exactly the **K=88** regime report 0006 Finding #1
...
2. *A real second objective.* The old objB (ordinal confidence) was near-degenerate
   ...
   exact register tension that drives config selection (report 0006: the conservative
...
- **Arms** (fixed/free split isolates what each tests):
  ...
  | 5 | discrete | p, q, r, ρ, λ | — | discrete vs refined (report 0005 ablation) |
  ...
  Arms 6–7 are the co-tuned structural test report 0006 Finding #3 requires: the
...
- **Resume guard:** refuses to run if `twm_nsga2.db` exists without `--resume`.

## 6. Selection protocol

This is the load-bearing discipline (report 0006): **tune on synthetic, decide on
real.**
...
**not** survive and are withdrawn in report 0006: "NSGA-II *selected* good constants"
...
2. **Co-tuned structural NSGA-II arm** — **done 2026-06-23/24** (arms 6–7). Verdict:
...
4. **objA/objB redesign + degeneracy gate** — **done 2026-06-20.** objA = production
   ...
   re-sweep.**
5. **Process fixes** — Optuna pinned (the lab's pinned Optuna requirement); fingerprint
   ...
   together (deferred until the NSGA-II work is complete).
...
2. **[SHARPENED · HIGH-for-bass] B is never estimated — and the mismatch model has the
   ...
   report 0005-gated step). The synthetic injects per-instrument + per-note B mismatch
   ...
   **Bass separability false-lock collapses 27% → 1.5% from a mere ~6%-mean B
   ...
   reframes report 0006's "structural fixes fail on bass": they fail because they tweak
   *scoring* while the bass bottleneck is the *template's B*. **Caveats (it is an
   ...
   > write-up + cross-tab in report 0006 (B-residual item). The real bass-B refit itself
...
3. **[UPGRADED → HIGH · objective design — FIXED 2026-06-20] Objective A optimised the
   ...
   **Resolution:** objA was retargeted to the production K=3 `prod_false_lock` (§4),
   ...
   `twm_nsga2.db`/`twm_pareto_arm*.json` were generated against the OLD (broken) objA —
   ...
   refinement** that report 0006 Finding #1 measured as the *worst* real setting (61/87),
...
8. **[CLEARED-in-part · front end] Peak *contract* is faithful; only detection noise is
   ...
   (`nsga2_evaluator.rs:391-394`); `peaks.csv`'s 128 is a pre-cap diagnostic scratch,
...
12. **[CONFIRMED → FIXED 2026-06-20 · robustness] `unwrap` on NaN comparisons.** All
   score comparisons in `nsga2_evaluator.rs` now use `f32::total_cmp` (a total order),
...
14. **[NEW · MED — partially addressed 2026-06-20] Auditability: the evidence chain is
   unversioned.** Optuna now pinned (the lab's pinned Optuna requirement, `optuna==4.9.0`).
   ...
   **asserts it == `e11fea90889dee30`** before running (a drifted dataset is refused,
   ...
   doc, `twm_nsga2.db`, the Pareto JSONs, and the real captures (`diagnostics/` is
   gitignored) remain **untracked** — they must be committed *together* so the
   evidence chain is reproducible (the user is deferring the commit until the NSGA-II
   work is complete). Nothing yet binds a *resumed* `twm_nsga2.db`'s existing trials to
...
If a real-world result contradicts the NSGA-II ranking, suspect — in order — (1) the
