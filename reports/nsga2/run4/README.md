# Post-remediation NSGA-II re-sweep — ARCHIVE

These are the TWM NSGA-II Pareto fronts (`twm_pareto_arm1..7.json`) and the Optuna study
DB (`twm_nsga2.db`) generated at the repo root by the **corrected** `tuner-lab/scripts/optimize_twm.py`,
archived 2026-06-30 to keep the repo root clean now that the TWM parameter experiment
is concluded.

**Distinct lineage from `nsga2_run{1,2,3}_backup/`.** Those three are the *pre-remediation*
...
See the methodology in `reports/nsga2/README.md`, and report 0006.
...
To regenerate fresh fronts, re-run `tuner-lab/scripts/optimize_twm.py` (writes back to the repo root).
