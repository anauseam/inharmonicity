...
**Series:** faithfulness audits (status table in `faithfulness-audit-01-twm.md`), item 6 of 8.
**Date:** 2026-07-04.
...
σ_B = 0.157/0.116 constants used by `nsga2_evaluator.rs`,
`joint_b_refine_diagnostic.rs`, and `reports/nsga2/README.md`.
...
| 5 | Bass-domain validity | documented limitation, not a deviation (report 0006: real upright bass B is 7–25× this prior) |
...
(the prior over-estimates only mildly in treble; report 0006).
...
default (report 0006, `validate_mat`) — a model-domain limitation of any fixed
...
per-note relative scatter ×(1 + σ·N(0,1)) in the NSGA-II synthetic generator
(`nsga2_evaluator.rs:254`, "Rigaud Fig. 3 split"), as the ±n·σ log-grid
bounds in `joint_b_refine_diagnostic.rs`, and cited as "[Rigaud Fig. 3]" in
`nsga2/README.md` §How-the-synthetic-is-built. **The paper's Fig. 3 is a
...
1. `get_expected_beta` doc-comment rewritten: paper's Eqs 7–8 named; MIDI →
   ...
   piano-specific); pointer to report 0006's bass-domain caveat.
2. `nsga2_evaluator.rs` σ comment: "Rigaud Fig. 3 split" → ours, with the
   audit pointer.
3. `joint_b_refine_diagnostic.rs` σ helper comment: same correction.
4. `nsga2/README.md` §synthetic: "[Rigaud Fig. 3]" → "(our calibration;
...
`nsga2_evaluator.rs` header already named. The split point corroborates it
...
2. **σ_B measures model-vs-data residual, not per-note physical scatter.** It is
   ...
   off the two-bridge curve," which is exactly the mismatch the NSGA-II synthetic
   wanted; it is not a claim about repeat-measurement noise. Our own measured
   repeat-noise figure is the separate σ_lnB(n) of report 0009.

**An unlooked-for corroboration of report 0009.** Rigaud's σ_B is the dispersion of
...
fallback almost exactly. So the report 0009 claim that σ_p is instrument-specific
...
top two octaves, and report 0009 analysis 7 shows no local measurement can check it
