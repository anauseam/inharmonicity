...
**Series:** faithfulness audits (status table in `faithfulness-audit-01-twm.md`), item 5 of 8.
**Date:** 2026-07-04.
...
- Mounir, Karsmakers & van Waterschoot (2021). "Musical note onset detection
  ...
  `inverse_participation_ratio` cites (`s13636-021-00214-7.pdf`).
...
- Noted, not applicable: Mounir et al. (2025) EURASIP howling-detection paper
  ...
  **rejected** alternative per report 0003, not current bases.

**Scope:** `tuner-core/src/algorithms/metrics.rs` (`rms`, `ema`, `nhwrsf`,
`inverse_participation_ratio`) plus usage/thresholds in `gatekeeper.rs`. **No changes applied** —
...
| 3 | `inverse_participation_ratio` (named `ninos2` when audited) | (c) **misattributed** — implements a Hurley–Rickard-family sparsity ratio, *not* Mounir's NINOS² (any variant); mechanism sound for its purpose |
...
**3. The metric is not NINOS² — the audit's second misattribution (after
...
(then `ninos2`, with its `ninos2_*` config and telemetry fields) perpetuates the
misattribution; renaming touches gatekeeper fields, diagnostics CSV headers,
and the GUI — **user's call**. It has since been done; the naming note below
carries what it took and where it landed.
...
`sustain_stability_threshold = 10.0`, `sustain_ema_alpha = 0.5` — all ours,
...
2. **The honest head-to-head is the sparsity cores, and neither dominates —
   ...
   source-property of this project's treble, cf. report 0006).
...
1. `inverse_participation_ratio` doc-comment rewritten: formal identity stated
...
3. `ema`: α-convention swap vs Giannoulis, cold-start sentinel rationale,
   ...
   all documented.
4. **Naming — the metric is named for the quantity, everything downstream for
   the gate.** The shipped metric is `metrics::inverse_participation_ratio`:
   `N/N_eff` is the *inverse* participation ratio, so a name asserting the
   participation ratio itself would be off by the inversion — high would read as
   "many bins participating" when the code means the opposite. It has two call
   sites, so the long formal name costs nothing.

   Everything downstream is named `sustain_*` for the gate it serves — the
   threshold, the EMA, the telemetry field, the GUI calibration panel — because
   the gate's job is stable while *which* sparsity measure serves it best is the
   open question this audit's A/B addendum raises. Tying the config surface to a
   measure that may be swapped is what made the earlier names cost so much.

   That surface is ~70 sites, and the one that matters is the profile's
   stability threshold, which is **persisted**. It carries a `serde` alias per
   earlier spelling (`ninos2_stability_threshold`,
   `participation_stability_threshold`), asserted in
   `tuner-core/tests/profile_schema_compat.rs`, so no operator's calibration is
   reset by a rename. Capture dumps were never affected — `analysis.json` has no
   such field. The faithful Mounir implementations in `tuner-lab` keep the name
   `ninos2`, because those genuinely are NINOS², and so does any sentence saying
   our metric is *not* Mounir's.

**Doc-bloat trim (2026-07-04, user feedback):** the audit's in-code comments
had drifted into narrating audit history (A/B statistics in the sparsity metric,
...
## Addendum (2026-07-15) — ℓ²/ℓ¹ citation pinning

**Motivation.** The 2026-07-04 fix relabeled the metric as ours and cited
...
`inverse_participation_ratio` doc-comment rewritten: formula line extended with = N·(ℓ²/ℓ¹)²;
...
No behavior change (comment-only). The rename stayed deferred at this point;
where it landed is item 4 above.
...
`resources/moba/`); then 7 (MAT re-check), 8 (Goertzel). The series continues
