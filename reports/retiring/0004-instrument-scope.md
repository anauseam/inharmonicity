# report 0004 — the three assumption layers behind the scope

The scope decision itself is an `ARCHITECTURE.md` Decisions entry; this is
*why* the boundary falls where it does. The
pipeline encodes three independent layers of physical assumption — excitation
(Gatekeeper), partial model (Engine + Worker), task (the tuning workflow) — and
each generalises differently. The scope is the intersection where all three
still hold, which is what makes the boundary principled rather than a list of
instruments someone happened to think of.

This is an argued derivation, not a measurement: there is no population and no
pre-registration, and it should not be cited as evidence about any instrument.
...
means replacing subsystems, not parameterizing them. This report fixes the scope so that
...
harmonic decay: NHWRSF detects the strike, sustain stability identifies the "Golden Window" of
...
- **Out of scope (deferred, architecturally anticipated):** modal percussion
  ...
  need wholesale replacement. Any future support is a separate decision.
