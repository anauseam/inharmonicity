# report 0015 — the ambient-σ gates, measured

Two pre-registrations in one document, which is why it is the longest thing here.
§§1–3 were written and fixed **before** the Task-1 measurement ran; §§4–11 are its
results. §12 is a *second* pre-registration, written before its own run, and §13
is that result — Candidate C, the per-bin startup floor, which passes on register
shape and **fails its own false-alarm guard**.

Two things this document does that are worth copying. It declares
**measurability in advance** (§1) — saying which of the two σ quantities the
harness can actually see before claiming either. And §13 records a *negative*
result in full, including the fact that the original instinct to rule out a
startup floor was right for a reason nobody had given: what defeats it is that a
2 s recording cannot calibrate a $10^{-3}$ tail, not that the decay outruns it.
The distinction matters, because it says the fix is **more reference samples per
decision**, not fresher ones.

**Open questions are in `## Open for decision` below, and they are live.** The
head records what was decided; it does not close these.

## Where this stands

**Task 1 measured (2026-08-22); the replacement gate is not decided here.**
Sections 1–3 were written and fixed *before* the measurement ran
([the methods standard](../../README.md) §1);
...
Supersedes nothing. This report is now the **only** record of the "Neyman–Pearson
...
[report 0014](../0014-unison-panel-against-isolation-truth.md) §8a/§8b**, whose
...
added here**: it
is the closest analogue to report 0011's control (an ambient threshold on 8192-sample
FFT bins), it is the one that feeds every lock baseline, and — unlike #2 and #3 —
it is a *scan-then-threshold* detector, so report 0011 §5's search-loss correction
applies to it and to neither of the others.

### What is already established

- **report 0011 §4** — the same ambient threshold, run as a control against FFT bins
...
- **report 0014 §8a** — gate #3 measured directly on six 5 s treble captures: the
...
### Why σ itself is not a number this report tries to get right
...
1. **Room noise is not white.** It is rumble-dominated — `audio.rs:83` records the
   DC blocker's corner at 35 Hz, *above* A0, and a later pass measured the coarse read
...
[`CONTRIBUTING.md`](../../CONTRIBUTING.md) already names this case: a threshold-dependent question has no
...
flagged **resolution-marginal**. This is the same inequality report 0011 records for
...
claim, in the same direction report 0014 §8a argued.
...
The live cut of **3** is anchored, not invented: report 0014 §8a measured the treble
gate closing while partials were still 3–15× above the noise beside them, so 3 is
the bottom of an already-measured range on this project's own data (the
anchor-a-threshold rule, [`CONTRIBUTING.md`](../../CONTRIBUTING.md)). The
...
Per `capture-sets.md`'s consumption rules; captures are validation-only and cannot select a
configuration ([`CONTRIBUTING.md`](../../CONTRIBUTING.md)).
...
`capture-sets.md` sanctions for questions a 1.5 s record cannot answer, and this section is the
...
The control exists because the first revision of this report did not have one, and
...
49 % on the extended set. The pre-registered figure is ~1.8× inflated. report 0014
...
**3.3×**, high **1.4×**. On report 0014 §8a's own six keys and 30 captures:
treble **2.9×**, high **2.2×**.
...
**Why this reverses one of the ambient-σ pre-registration's own conclusions.** The prompt ruled out a
...
report 0011's reference-cell floor were needed, and it means **no same-window
...
- **n = 2 instruments.** Validation only (`capture-sets.md`; the two-instrument
  rule in [`CONTRIBUTING.md`](../../CONTRIBUTING.md)). These figures
...
3. ~~References to the deleted `suspected-issues.md` dangle.~~ **Resolved
   ...
   rewritten, and the prose in reports 0011, 0012 and 0014 kept in its historical
   tense ("then recorded in…") with a link to this report appended, so the record of
...
Written **before** the run, per [the methods standard](../../README.md) §1. §8 claims
...
the hot path. n = 2 instruments cannot select ([`CONTRIBUTING.md`](../../CONTRIBUTING.md)), and `CLAUDE.md`'s
...
| CFAR local reference | ✓ (report 0011) | ✓ analytic | **no reference cells exist** |
...
`N > 4·fs/f₀` that report 0011 recorded below F3.

**The pre-registration's original instinct was right, for a reason it did not give.** It ruled
...
The extended set is not in the repository; `capture-sets.md` says where it lives, and §5 pins
...
- [report 0011](../0011-coarse-spectral-readout.md) — the CFAR port that ships in the
  coarse readout, and the search-loss lesson that applies to gate #1 alone.
- [report 0014](../0014-unison-panel-against-isolation-truth.md) §8a/§8b — the first
