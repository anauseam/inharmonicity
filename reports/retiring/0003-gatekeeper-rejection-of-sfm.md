# report 0003 — the ΔSFM plateau, and what the Gatekeeper's wait actually costs

Two measurements, three months of tree apart: the 2026-06 investigation that removed
SFM, and the 2026-09-02 amendment that priced the wait the resulting architecture
settled on.

The amendment is the more interesting one, because it **refutes a claim this
document itself made**. The Consequences section below asserted the new gate
adapts to each note's register; it does not, and the `S` markers say so directly.
The claim is struck through where it appears.
...
| sustain-stability ramp-up           | 3      | ~139ms     |
| **Total**              | **5**  | **~232ms** |
...
robust, geometrically stable onset detector for this application, and that sustain stability
...
hops). It prices the wait this report's architecture settled on, which nothing had
...
[report 0010](../0010-m-of-n-lock-rule-replay.md)'s M-of-N window rather than to SFM —
...
- The Gatekeeper architecture is radically simplified and relies purely on:
  ...
  3. **sustain stability** (Am I a stable tone?)
...
- The old 10-frame static wait (~464ms) is confirmed as a conservative but not incorrect design. The new NHWRSF + sustain-stability architecture achieves ~232ms dynamically, adapting to each note's register and transient character rather than using a blind stopwatch or unobservable SFM derivatives.
