...
- **Caveat**: CV assumes independent errors; serially correlated errors bias
  λ low (the report 0009 chain-noise item measured exactly this and found
  r ≈ 0.15, mild).
...
| 7 | `cv_masked` | (b) OURS, documented (report 0007; pseudo-points shape the smoother but are excluded from the score — verified in code) |
...
h_ii) but are skipped in the score — exactly the documented report 0007
...
independent errors (serial correlation biases λ low). report 0009 measured
