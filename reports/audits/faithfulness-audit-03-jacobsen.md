...
**Series:** faithfulness audits (status table in `faithfulness-audit-01-twm.md`), item 3 of 8.
**Date:** 2026-07-02.
...
- The Worker/MAT path is **unaffected** (it uses CSPE — audited faithful,
  audit 02).
- The 74/87 baseline, the NSGA-II-tuned TWM constants, and the
  ...
  (the bias was present in both the synthetic NSGA-II harness and the real
  captures, so the tuning was at least self-consistent — but the fixed
  estimator may shift both the baseline and the optimum).
  *[Correction 2026-07-05: the parenthetical's synthetic half is
  wrong — `nsga2_evaluator` synthesizes `SpectralPeak`s directly with an
  ...
  survive the fix (75/87 vs the default's 77/87) — see report 0006 items 2/5,
  2026-07-05 entry.]*
...
- **Consequence: every pre-fix number is stale** — the 74/87 baseline, the
  ...
  biased peaks. Re-derivation was done on 2026-07-05 — results
  in report 0006 items 2 & 5 (the t1898 candidate did not survive the fix).
...
5. Re-validation: re-run `tuner-lab/scripts/validate_config.py` over the 87 captures to
...
- **The variance expression is genuinely new capability, but not for now.**
  ...
  M&B), exactly the bolt-on class report 0006 gates on the second instrument;
