...
[`../internals/layering.md`](../../docs/internals/layering.md),
...
> questions belong to the reports and to `ARCHITECTURE.md`; the treble asymptote's
...
Six audits — 02 and 05–09 — are staged in [`../retiring/`](../retiring/README.md)
while their site pages are written: five cover the stable subsystems, and 09
covers Rigaud's piano model. The links below follow them.

## Status — 14 audits, all complete
...
| 2 | `spectral.rs` `cspe` vs Short & Garcia 2006 | **Faithful** — caller contract verified; doc-only fixes (stale Hamming→Hann) | [02](../retiring/faithfulness-audit-02-cspe.md) |
...
| 4 | `peaks.rs` (`extract_peaks`, `mask_peaks`) | **Fixed cites** — `mask_peaks` Gómez citation fabricated → reclassified validated-bespoke (report 0002); phantom "Miron 2014" removed; Cano 40→30 dB documented | [04](faithfulness-audit-04-peaks.md) |
| 5 | `metrics.rs` (gatekeeper metrics) | **Fixed cites** — `inverse_participation_ratio` misattributed → relabeled ours (N/N_eff); nhwrsf lineage + band de-hardcoded; A/B built (`sparsity_ab`) | [05](../retiring/faithfulness-audit-05-metrics.md) |
| 6 | `models.rs` `get_expected_beta` (Discovery B prior) vs Rigaud 2013 | **Faithful form** — treble constants = paper's universal fit exactly; bass = ours by design; σ_B traced to the DAFx-11 precursor §3.2 (2026-08-27 addendum reverses the "false attribution" verdict) | [06](../retiring/faithfulness-audit-06-b-prior.md) |
| 7 | `mat.rs` re-check vs Hodgkinson DAFx-09 | **Faithful** — all Eq/§ citations verified except phantom "§7" (→ Conclusion §4); OUR-constants documented | [07](../retiring/faithfulness-audit-07-mat.md) |
| 8 | Goertzel + phase-vocoder tracking in `engine.rs` | **Faithful** — recurrence textbook-correct; phase-offset constraint documented; NEYMAN_PEARSON_K re-derived exact | [08](../retiring/faithfulness-audit-08-goertzel.md) |

**Tally:** 1 real bug fixed (jacobsen), 5 citation defects corrected, 2
validated-bespoke reclassifications (`mask_peaks`, `inverse_participation_ratio`); the load-bearing
...
[report 0006](../0006-discovery-refinement-validation.md).

## The tuning-curve modules (2026-07-15)

The 2026-07 tuning-curve implementation introduced four new paper-based
modules; audit 05 had also left `inverse_participation_ratio` with a family-level citation. All
...
| # | Subject | Verdict | Doc |
| --- | --- | --- | --- |
| 5-add. | `metrics.rs` `inverse_participation_ratio` ℓ²/ℓ¹ citation pinning | **Pinned** — H&R *define* the measure (Table I + Thm 4.1); ours = N·(ℓ²/ℓ¹)², satisfies all six criteria (D4/P2 from the ×N factor); **H&R's own Thm A.4/Table III D3 entry shown erroneous** (contradicts their Thm A.19; derivation on record); Hoyer §3.1 + Bell & Dean 1970 cites added | addendum in [05](../retiring/faithfulness-audit-05-metrics.md) |
| 9 | `algorithms/rigaud.rs` vs Rigaud 2013 | **Faithful** — Eqs 7–9/20/29–31 + treble pair + A&S erf all exact; 1 mis-citation fixed (§III.B.3 → §III.A.3.b); paper's s_B sign typo noted in-code | [09](../retiring/faithfulness-audit-09-rigaud.md) |
...
| 12 | `algorithms/curves.rs` — assembly audit | **Clean** — every Eq/§ claim verified (incl. §IV.C.2 `min` typo confirmed via Fig 9), every OURS component report-linked, constants sweep clean; 2 Eq-10→10/11 cites + 1 stale GCV comment fixed | [12](faithfulness-audit-12-curves.md) |
...
go to the audit of `algorithms/rigaud.rs`.

## The coarse readout (2026-07-26)

The first audit of code written *after* waves 1–2: the OS-CFAR gate behind the
coarse spectral readout (shipped 2026-07-25, report 0011). Primary sources in
`resources/tracker/`.
...
- **(c) undocumented deviation / bespoke addition** — flag it, then either
  document it (with its provenance and validating report) or remove it.
...
citation- or comment-level, but do not assume that — the first round's real bug
(`jacobsen`) was in confidently-written code, and the coarse-readout audit deleted a constant.
