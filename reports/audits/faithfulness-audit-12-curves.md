...
**Series:** faithfulness audits (queue table in `README.md`),
...
`docs/design/tuning-curve-design.md` (the governing spec, in git history at `b45adc0`), reports 0007–0009.
...
| 6 | ln-B shrinkage block (`SIGMA_*`, `sigma_prior`, blend) | (b) OURS/measured, report 0009 — conjugate-normal posterior-mean form re-checked; 1.4826 = normal-consistent MAD scale, documented |
| 7 | `REVERSION_LENGTH_KEYS` + `reversion_weight` | (b) OURS/derived, report 0007 — Euler–Lagrange re-derived: w₀z + λz⁗ = 0 ⇒ decay rate (w₀/λ)^¼/√2 ⇒ w₀ = 4λ/ℓ⁴ exactly |
| 8 | §2 detector (pre-exclusion + `finish` flags) | (b) design-note §2, flag-and-exclude semantics verified (never clamps) |
| 9 | Minimum-norm chain gauge | (b) OURS/derived, report 0007 (mean-centering over measured keys; tested) |
...
| 11 | `select_rho_reg_weight` — LOO + 1-SE | (b) OURS, ESL §7.10 cited, report 0008; SE = sd/√n standard |
...
| 14 | Stale "which GCV re-selects" comment | (c) **stale doc → FIXED** (GCV retired in report 0008; → "the CV") |
...
power law and σ_p self-calibration carry their report 0009 provenance
...
their report 0008 record; pseudo-rows shape the system and are excluded from
...
in the unit-mean gauge comment (GCV was retired for row-LOO + 1-SE in report
0008; the gauge argument itself survives unchanged under the CV).

**15. Constants sweep.** SIGMA_LNB_COEFF/FLOOR, SIGMA_PRIOR_* (report 0009,
measured, labeled); 1.4826 (MAD consistency, documented);
NEGATIVE_STRETCH_TOL_CENTS = 0.01 ¢ (FP-noise guard, documented);
REVERSION_LENGTH_KEYS = 12 (derived, report 0007); GIORDANO_MIN = 8 (§VI.C,
...
Form-2 sensitivities per report 0008); 440.0/1200/100 (definitions); 1e-12
...
1. Two Eilers "Eq. 10" → "Eqs. 10/11" citations (`multi_interval` doc +
   the row-LOO comment).
2. "which GCV re-selects" → "which the CV re-selects" (stale after report
   0008).
