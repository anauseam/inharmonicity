...
**Series:** The first audit of code written
*after* waves 1–2 — the coarse readout shipped 2026-07-25 (report 0011).
**Date:** 2026-07-26.
...
OS-CFAR gate in the same file — the unison line estimator's, report 0012 — makes the
...
claim on this paper and are handled separately.
...
| 9 | Search-loss `P_fa / M` correction | (b) ours, documented (report 0011 §5) — the paper's P_fa governs one CUT |
...
`peaks.rs` documented the shipped quantile as "Ours, measured (report 0011) —
...
whole keyboard — and the median's measured failure (±400 ¢ junk, report 0011 §4) is
...
(Fig. 10) — and report 0011 §5's realized-P_fa figures are the measurement of it
...
0.0013 — the 17–32 bucket identical to report 0011 §5's recorded 2.7×); 2048 stays
...
report 0011 §4, `peaks.rs`, and the harness all said the wide flank "lets a low
...
(spacing under 115 Hz, ≈ key ≤ 25) and inert above. the original
...
| bins | 32 bins @ 8192 | — | a different physical width at each FFT size — already rejected in report 0011 §4 |
...
- **Search loss.** Rohling's P_fa governs one cell under test; this detector
  ...
  the paper; measured closure in report 0011 §5.
...
**No value changed**, and one constant was removed. report 0011's measurements
...
- report 0011 §4: quantile and guard entries annotated with this audit; flank-floor
  mechanism corrected.
