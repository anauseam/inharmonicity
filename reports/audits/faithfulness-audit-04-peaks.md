...
**Series:** faithfulness audits (status table in `faithfulness-audit-01-twm.md`), item 4 of 8.
**Date:** 2026-07-04.
**Sources checked:**
...
- report 0002 (`0002-twm-peak-masking-validation.md`, 2026-05-28) — the mask's
...
| 4 | `mask_peaks` doc says −40 dB; constant is −30 dB | (c) internal inconsistency — the **comments** are wrong (report 0002 validated −30) |
| 5 | Cano 1998 "SMS dynamic range" attribution | **resolved** — Cano §4.3 does specify 40 dB; ours is a documented 40→30 dB adaptation (report 0002) |
| 6 | `mask_peaks` mechanism itself | **(b′) validated bespoke heuristic** — reclassify honestly, keep (report 0002: 8/8, replaced the failed geometric gate) |
...
says). report 0002 records the mask as introduced and validated at **−30 dB**, so
...
that report 0002 validated. Final classification of the global gate: **(b)
...
- has a real empirical record: **report 0002** (2026-05-28) — introduced to
...
1. `mask_peaks` doc-comment rewritten: provenance section (ours, report 0002
...
2. −40 dB prose corrected to −30 dB at both sites; constants unchanged and
   now labeled "OURS, report 0002-validated".
3. internals/04: phantom "Miron 2014" replaced with Short & Garcia 2006, and
   ...
   the mechanism is ours, cite the validating report instead).
