# report 0011 — the coarse spectral readout

Eleven numbered measurements. The three-way comparison in Context was fixed
**before** the tracker was scored, which is what lets §§1–2 conclude that the
window fix does not remove the need for the readout rather than the reverse.

Two results here generalise past their own section. §7's fixed $n^\ast = 4$ below
key 16 was forced by a **cross-instrument disagreement at $n = 5$** — the guitar
and the pianos wanted different partials, which is the kind of thing one
instrument cannot show. And §§10–11 (added 2026-07-26) price the band-slope
window: the lag is exactly $T/2$, but the jitter is *inelastic* in $T$, so
tripling the window buys 1.8× rather than 5.8×.

## Where this stands

**IMPLEMENTED 2026-07-25.** Five measurement rounds settled every
constant and rule before any hot-path code was written; this report carries the
...
## The measurements that forced the decisions

Decided in report 0011.
...
full measurement are now [report 0015](retiring/0015-ambient-sigma-gates-measured.md).
...
Added by a later measurement pass. `BAND_READABLE_HZ = 18.0` shipped as "the 21.53 Hz
...
A later measurement pass. `BAND_SLOPE_WINDOW_SECS = 0.6`, `BAND_SLOPE_MIN_SPAN_SECS = 0.25`
...
(1–2.5 ¢) include real rate drift, so they are not a contradiction of this report's
...
value had no citation, no record and no measurement behind it, and as an absolute
...
5. report 0009 — σ_lnB(n) and the B-shrinkage the cold-start references inherit.
6. `docs/design/strobe-and-manual-tuning-ui-design.md` §5.5 / D4 (git history, `b45adc0`) — the readout
   pair this report completes.
