# report 0008 — Giordano-layer fidelity and the derived interval weights

Curve review sets 2–3.

Two of the derivations below are load-bearing in a way a one-line record cannot
show. The
**1-SE rule** is not decoration: the LOO-CV curve for the ρ fit is flat at noise
level across four decades, so the bare argmin is choosing among indistinguishable
options and lands on a grid edge that drives engine (c)'s A7 to +47 ¢. And the
**§VI.C gate** is quoted from the paper verbatim, because the number it yields
(8 coincident pairs) is a bass-derived conservative floor adopted compass-wide,
not a fitted threshold.

Consequences are one instrument, 87 captures — diagnostics, not selection
evidence.
...
(`docs/design/tuning-curve-design.md`, in git history at `b45adc0`, is the governing spec; Set 1 is
report 0007). Everything below was measured on the 87 regenerated captures —
...
## Set 2 — the coincidence-bracket scan window, derived

Decided in report 0008.
...
## Set 2 — the §VI.C sufficiency gate, verified against the paper
...
## Set 2 — why the 1-SE rule is load-bearing for the ρ fit
...
* The bare argmin picked the weak grid edge (w = 0.01, 2.5 % "better" than
  ...
  the same defect class as report 0007's boundary reversion: an un-scored,
...
* The 1-SE rule (verified: Hastie–Tibshirani–Friedman, *ESL* 2nd ed.
  ...
  report 0007's Eilers citation) selects w = 10: φ = (κ 3.146, m₀ 57.5,
...
### λ selection: why GCV was retired for weighted fast-LOO + 1-SE
...
* Giordano §VI.C quoted from the PDF this session (gate + shift rule +
  ...
  secondary sources (primary is a textbook; same status as report 0007's
...
* Design note: `docs/design/tuning-curve-design.md` §§2, 3.2, 6, 13 (git history, `b45adc0`).
* report 0007 — Set 1; the reversion and gauge mechanics Set 3 interacts with.
