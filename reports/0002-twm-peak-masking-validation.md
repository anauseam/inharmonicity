# report 0002 — the −30 dB peak mask, first full-compass validation

Measured 2026-05-28 on the eight captures then held in `diagnostics/`.

This is the project's earliest measurement report and it predates the methods
standard in [`README.md`](README.md). It has no pre-registered decision rule and
no null — the run was a characterisation, and its result (8/8, zero false locks)
is a *floor*, not a discrimination test. Later work treats it that way: report 0006
re-derives lock accuracy against a stated baseline, per register, with a McNemar
test.
...
- NSGA-II parameter tuning (`q`, `r`, `ρ`) against a synthetic dataset to further
  tighten the error margin between the winning key and the runner-up.
  See [report 0001](0001-nsga2-tuning.md) for the methodology.
