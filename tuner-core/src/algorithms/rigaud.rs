...
//! sites.
//!
//! **Fundamental convention:** the stiff-string law
...
//!    JASA 133(5), pp. 3107–3118. DOI: 10.1121/1.4799806.
...
impl BXi {
    /// The medium-piano default bass pair: [`crate::models::get_expected_beta`] in
    /// the MIDI domain (n = m − 20), and the fallback when there is nothing to fit.
...
///
/// `points` are `(m_midi, b)` pairs from every trusted key: the L1 norm is the
/// paper's outlier guard, and the fixed treble asymptote gives treble points little
/// pull on the bass pair. `None` with fewer than two points or a non-positive `b`.
///
/// The objective is the paper's; the optimizer, a coarse-to-fine grid search over
/// s_B ∈ [−0.20, 0] and y_B ∈ [−14, −2], is ours. The paper initializes s_B at
/// −8.9e-2: §III.A.3.b prints +8.9e-2, a sign typo, as Fig. 1(a)'s bass asymptote
/// falls with m.
// The domain brackets the paper's pianos and the measured upright's (−0.050, −6.3).
...
/// Upper edge of [`fit_rho_phi`]'s κ search domain, and so of the octave types the
/// fit can express (ρ ≤ κ + 1).
// Ours: it brackets the paper's Fig. 9 range and both pianos' calibrations.
...
///
/// The L1 term is the paper's; the penalty is ours, because ρ points from
/// per-octave dissonance scans are sparse and noisy. `reg_weight` is in ρ-units per
/// normalized parameter unit², with m₀ and α normalized by an octave (12 MIDI
/// steps); `0.0` gives the paper's pure Eq. 31.
///
/// `points` are `(m_midi, rho)` pairs; `None` when empty. A coarse-to-fine grid
/// search over κ ∈ [0, [`RHO_FIT_KAPPA_MAX`]], m₀ ∈ [21, 108], α ∈ [5, 80].
...
mod tests {
    ...
    /// erf against A&S table values, to the approximation's own
    /// 1.5e-7 bound.
    #[test]
