...
impl SeismographViewer {
    pub fn new(history: Vec<f32>, noise_ceiling: f32) -> Self {
        Self { history, noise_ceiling, cache: canvas::Cache::default() }
    }
...
impl<Message> canvas::Program<Message> for SeismographViewer {
    ...
    fn draw(
        ...
        let geometry = self.cache.draw(renderer, bounds.size(), |frame| {
            if !bounds.width.is_finite() || !bounds.height.is_finite() { return; }
            ...
            let max_val = self.history.iter().copied().fold(self.noise_ceiling * 1.5, f32::max).max(1.0);
            ...
            let p = builder.build();
            frame.stroke(&p, canvas::Stroke::default().with_color(Color::from_rgb8(0x34, 0x98, 0xDB)).with_width(2.0));
            ...
            frame.stroke(&t_line, canvas::Stroke::default().with_color(Color::from_rgba8(0xF3, 0x9C, 0x12, 0.8)).with_width(1.5));
