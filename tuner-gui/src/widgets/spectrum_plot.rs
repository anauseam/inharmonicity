//! # Spectrum plot
//!
//! The live magnitude spectrum as a bar chart: one bar per FFT bin, on a 60 dB
//! scale below the frame's strongest bin. A spectrum, not a spectrogram: it
//! shows the current frame only, with no time axis.
...
/// Canvas program plotting one magnitude spectrum, each bar one FFT bin.
...
impl SpectrumPlot {
    /// Creates a spectrum plot over `data`, an FFT magnitude spectrum.
    ...
    /// Creates the view element for the spectrum plot.
    pub fn view(self) -> Element<'static, crate::Message> {
        Canvas::new(self).width(Fill).height(Fill).into()
    }
}

impl<Message> canvas::Program<Message> for SpectrumPlot {
    type State = ();
    ...
    fn draw(
        ...
        let geometry = self.cache.draw(renderer, bounds.size(), |frame| {
            ...
            // At least 1 px wide, or a bar may not render at all.
