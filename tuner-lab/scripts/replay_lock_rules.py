...
Pure post-processing: no engine changes. Three subcommands:
  ...
  cache    Run `gatekeeper dump` + `engine dump` (DEFAULT TwmConfig,
...
Usage:
  python3 tuner-lab/scripts/replay_lock_rules.py cache --base diagnostics_piano_1 \
      --out replay_cache/p1_discrete.csv [--refine] [--jobs 8] [--tidy]
  python3 tuner-lab/scripts/replay_lock_rules.py sweep replay_cache/p1_discrete.csv \
      [--n-max 43] [--surface-out replay_cache/p1_discrete_surface.csv]
  python3 tuner-lab/scripts/replay_lock_rules.py concord \
