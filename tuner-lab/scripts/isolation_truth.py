#!/usr/bin/env python3
"""Isolation-set truth side — every table in report 0014 that is not the panel's.
...
`cargo lab mat regen` output, the same split as `audit_captures.py`.
...
Sections map to report 0014: 1 the screen, 2 per-string truth, 3 the regime in
...
# report 0012 §4: above 1.6 x 2/T the estimator is exact to a -0.085 Hz systematic
# with sigma <= 0.009 Hz. C1's tolerance is derived from these plus the coupling
# bound this script measures -- never an invented percentage (CONTRIBUTING.md, "anchor a threshold").
EST_BIAS_HZ = -0.085
EST_SIGMA_HZ = 0.009
COUPLING_BOUND_HZ = 0.26   # measured in section 6: pooled median excursion

# The failed-solo screen (report 0014 §1): a failed capture disagrees with its own
...
def section5(panel, strings):
    ...
    if pop:
        ...
        print("  NOTE: that population is ONE key. See report 0014 §5 — underpowered,")
...
def section7(rows, strings):
    ...
    print("\n  A real B difference tilts the split across partials by "
          "866*(B1-B2)*n^2 cents,")
    print("  which is what report 0012 §6's residual-estimated standard error absorbs.")
