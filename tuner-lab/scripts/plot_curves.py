...
the per-sample gatekeeper/goertzel images:
    ...
    python tuner-lab/scripts/plot_curves.py curve_report.json [out.png]
...
numbers are diagnostics, not selection evidence: choosing a curve needs an in-tune instrument.
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python tuner-lab/scripts/plot_curves.py <curve_report.json> [out.png]")
        sys.exit(1)
