//! # NSGA-II evaluator: synthetic dataset and discovery fitness
//!
//! The harness behind the NSGA-II parameter sweep (report 0001; split discovery,
//! report 0005). Generates the fixed-seed synthetic piano dataset
...
//!
//! Physics grounding (sources cited inline below and in report 0001):
...
//! total ET deviation |D| is within [`AMBIGUOUS_CENTS`]; beyond that it lands in
//! the reported "ambiguous" bucket.
//!
...
/// Scored-set clamp, in cents from ET: just inside the ±80-cent refinement
/// window, so the heavily detuned, pitch-raise stratum is scored and labeled by
/// the intended key. Frames past it are unreachable and stay in the ambiguous
...
struct Frame {
    ...
    d_cents: f32,
    /// |D| past [`AMBIGUOUS_CENTS`]: generated and reported, excluded from objectives.
    ambiguous: bool,
...
fn emit_partial_cluster(
    ...
    while i < n {
        ...
        // occasional destructive nulls = the unison dropouts the sustain-stability metric sees).
...
fn gen_frame(rng: &mut Rng, piano: &Piano, key: usize, hard: bool) -> Frame {
    ...
    // Treble attack archetype: dense 69–128-peak post-attack frames of broadband
    // low/mid content, which treble keys fail on.
    ...
    // mask_peaks then filters and returns ascending order.
...
/// itself must become detuning-aware.
...
struct Acc {
    n: usize,
    false_locks: usize,      // separability, the K=88 argmin (diagnostic)
    prod_false_locks: usize, // production K=3 via discover(), all registers
    /// Production false-locks at K ∈ {2,3,4,5} (diagnostic K-robustness sweep).
    /// Index 1 is the shipped K=3 (== prod_false_locks); the neighbouring K measure
    /// the constants' K-sensitivity, since K=3 is empirical (best of {3,88} on
    /// n=1). Computed only in refine mode (0 in discrete).
    prod_fl_k: [usize; 4],
    margin_sum: f64,
    fid_sum: f64,
    fid_n: usize,
...
/// Folds one frame into `acc`: its production false lock (`discover()` at the
/// shipped TOP_K returns the wrong key), which per register gives the optimized
/// `fl_bass` and `fl_treble`; and its ordinal confidence, the fraction of the 87
/// impostors the true key out-scores, rank-based and so immune to error-scale
/// inflation and compression. The all-88 error scan also feeds the diagnostic
/// probes.
fn process_frame(
    f: &Frame,
    ...
    // Separability, the K=88 argmin: a diagnostic, not an objective. In refine mode
    // it is the exhaustive-refinement regime report 0006 Finding #1 measured as the
    // worst real setting, and its optimum is degenerate (the NSGA-II methodology).
    let false_lock = argmin != key;

    // Production false lock at the shipped TOP_K = 3 (discover(): Stage A top-K,
    // Stage B refine, argmin), which the optimized objectives count per register.
    ...
    // Stage-A scores for top-K ranking, exactly discover()'s selection rule: are the
    // chosen constants K-robust or fitted to K = 3? Refine mode only, as production
    // refines.
    ...
    // ── Ordinal confidence, emitted as `objB` ──
    // 1.0: the true key beats every impostor; 0: it is the worst.
    ...
    // Degeneracy probes, diagnostic only.
    let mut sorted = errs;
    ...
    acc.prod_false_locks += prod_false_lock as usize; // production K=3, all registers
...
/// single-threaded, so this is what keeps the NSGA-II loop in the hours, not days.
...
/// Dense-scan oracle for the pre-grid and golden-section constants: on a
/// subsample, compare `refine_scale` against a 0.5-cent dense
...
/// Oracle-B diagnostic (report 0005 revisit #3 / the NSGA-II methodology's §8.2 gate): how much
...
/// one-directional ≈ 7× bass gap measured on the real piano (the NSGA-II methodology).
/// So a large drop
...
/// `optimize_twm.py` minimizes `fl_bass` and `fl_treble`; `objA` (the production
/// K=3 false-lock rate over all registers), `objB` (mean ordinal confidence),
/// `sep_fl` (K=88 separability) and `prod_fl_k*` are diagnostics.
fn emit_json(acc: &Acc) {
    let n = acc.n.max(1) as f64;
    let obj_a = acc.prod_false_locks as f64 / n; // production K=3, all registers
    let sep_fl = acc.false_locks as f64 / n; // K=88 separability (diagnostic)
    let prod_fl = acc.prod_false_locks as f64 / n; // == objA, for older JSON readers
