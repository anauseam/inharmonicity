...
| 6 | `models.rs` `get_expected_beta` vs Rigaud | **DONE + FIXED 2026-07-04** (`faithfulness-audit-06-rigaud.md`) — model form faithful (Eqs 7–8); treble pair = paper's universal fit EXACT (re-index verified); bass pair = OURS by the paper's own design (now documented); **σ_B 0.157/0.116 "[Rigaud Fig. 3]" attribution FALSE** (not in paper — ours, corrected at all 3 sites) |
