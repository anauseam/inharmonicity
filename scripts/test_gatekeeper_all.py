...
def main():
    ...
    for key in keys:
        ...
        stable_idx = df.index[
            (df.index > first_onset) & (df["state_name"] == "Stable")
        ].tolist()

        if not stable_idx:
            results.append(
                {"key": key, "transient_frames": -1, "status": "Never Stable"}
            )
            continue
        ...
        results.append(
            {"key": key, "transient_frames": frames_waiting, "status": status}
        )
    ...
    if len(valid) > 0:
        ...
        print(
            f"Min Delay: {valid['transient_frames'].min()} frames ({valid['transient_frames'].min() * 46.4:.1f} ms)"
        )
        print(
            f"Max Delay: {valid['transient_frames'].max()} frames ({valid['transient_frames'].max() * 46.4:.1f} ms)"
        )
        print(
            f"Mean Delay: {valid['transient_frames'].mean():.1f} frames ({valid['transient_frames'].mean() * 46.4:.1f} ms)"
        )
        print(
            f"Median Delay: {valid['transient_frames'].median():.1f} frames ({valid['transient_frames'].median() * 46.4:.1f} ms)"
        )
